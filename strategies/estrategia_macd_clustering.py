...
from binance_client import conectar_binance
...
async def estrategia_macd_clustering(binance, context):
    try:
        ...
        df_config = pd.read_csv('config/cripto_tamanho_macd.csv') # 40 usdt com 10x
        ...
        for _, row in df_config.iterrows():
            ...
            # price = (await asyncio.to_thread(binance.fetch_trades, symbol))[-1]['price']
            # price = float(binance.price_to_precision(symbol, price))
            
            try:
                trades = (await asyncio.to_thread(binance.fetch_trades, symbol))
                if not trades:
                        ...
                        if df['RSI'].iloc[-1] >= 58 and df['RSI'].iloc[-1] <= 70:
                            if df['MACD_12_26_9'].iloc[-1] >= df['MACDs_12_26_9'].iloc[-1] and df['MACD_12_26_9'].iloc[-2] <= df['MACDs_12_26_9'].iloc[-2]:
                                if trend == 1 and t_stop > ama and side != 'short':
                                    ...
                                    await asyncio.to_thread(binance.create_order,symbol= symbol,side= 'buy', type='MARKET',  amount =posicao , params={'hedged': 'true'})

                                    # Segunda Entrada (limit)
                                    # await asyncio.to_thread(binance.create_order, symbol= symbol,side= 'buy', type='LIMIT',  amount =posicao / 2, suporte, params={'hedged': 'true'})

                                    # Stop
                                    await asyncio.to_thread(binance.create_order, symbol= symbol,side= 'sell', type='STOP_MARKET',  amount =posicao, params={'stopPrice': stop})

                                    # Take Profit
                                    await asyncio.to_thread(binance.create_order,symbol=  symbol,side= 'sell', type='TAKE_PROFIT_MARKET',  amount =posicao, params={'stopPrice': alvo})
                            ...
                            if df['MACD_12_26_9'].iloc[-1] <= df['MACDs_12_26_9'].iloc[-1] and df['MACD_12_26_9'].iloc[-2] >= df['MACDs_12_26_9'].iloc[-2]:
                                if trend == 0 and t_stop < ama and side != 'long':
                                    ...
                                    # Primeira Entrada (mercado)
                                    await asyncio.to_thread(binance.create_order,symbol=  symbol,side= 'sell', type='MARKET', amount = posicao, params={'hedged': 'true'})

                                    # Segunda Entrada (limit)
                                    # await asyncio.to_thread(binance.create_order,symbol=  symbol,side= 'sell', type='LIMIT', amount = posicao / 2, resistencia, params={'hedged': 'true'})

                                    # Stop
                                    await asyncio.to_thread(binance.create_order, symbol= symbol,side= 'buy', type='STOP_MARKET',  amount = posicao, params={'stopPrice': stop})

                                    # Take Profit
                                    await asyncio.to_thread(binance.create_order,symbol=  symbol,side= 'buy', type='TAKE_PROFIT_MARKET',  amount = posicao, params={'stopPrice': alvo})
